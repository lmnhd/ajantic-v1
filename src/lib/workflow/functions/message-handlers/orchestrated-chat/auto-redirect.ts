...
import { AGENT_FORM_reWriteRequestMessage } from "@/src/lib/post-message-analysis/agent-request-form-creator";
...
import { OrchestrationProps, ContextContainerProps, ServerMessage } from "@/src/lib/types";
import { UTILS_convertLineSetsToContext, UTILS_getModelsJSON } from "@/src/lib/utils";
import { UTILS_getModelArgsByName } from "@/src/lib/utils";
import { MODEL_getModel_ai } from "@/src/lib/vercelAI-model-switcher";
import { generateObject } from "ai";
import { z } from "zod";

export async function ORCHESTRATION_autoModeRedirect(props: OrchestrationProps): Promise<{
    nextAgent: string;
    ...
    console.log("!!!_Executing ORCHESTRATION_autoModeRedirect...!!!", props.autoProps?.currentMessage);
  let result = {
    nextAgent: "",
...
4. FALSE FOR: Any message that doesn't BLOCK workflow progress`),
  ...
  // Log state BEFORE infoRequest block
  console.log("Before infoRequest block - newContext:", JSON.stringify(result.newContext, null, 2));

  if (result.infoRequest) {
    // Log state immediately INSIDE infoRequest block
    console.log("Inside infoRequest block - initial newContext:", JSON.stringify(result.newContext, null, 2));

    if (!result.newContext || result.newContext.length === 0) {
      result.newContext = props.autoProps?.contextSets ?? [];
      result.contextRequest = true; // Should this be set here?
      console.log("Inside infoRequest block - after potential reset:", JSON.stringify(result.newContext, null, 2));
    }
      ...
      const tempCTX = [...result.newContext, UTILS_TEAMS_infoRequestContextFormSet(formSchema, [], props.currentAgent ?? {}, props.autoProps?.messageHistory ?? [], false)]
      result.newContext = tempCTX

      result.redirectToUser = true; // force redirect to user

    //console.log("Inside infoRequest block - BEFORE push:", JSON.stringify(result.newContext, null, 2));
    // result.newContext.push({
    //   setName: "Input Request Form",
    //   text: "test"
    // })
    // result.newContext.push({
    //   setName: "Input Request Form",
    //   text: "test2"
    // })
    console.log("Inside infoRequest block - AFTER push:", JSON.stringify(result.newContext, null, 2));
  ...
  // Log the final result being returned to the client
  console.log("autoRedirectOrchestrator3 returning to client:", JSON.stringify(result, null, 2));
  // result.newContext.push({
  //   setName: "Input Request Form",
  //   text: "test3"
  // })
...
export async function TEST_ORCHESTRATION_autoModelRedirect(){
    return 'working!'
}
