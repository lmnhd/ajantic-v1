...
import { pollyGetVoiceURIFaster, whisperGetVoiceURI } from "./voices";
import { AppContext } from "@/src/app/context";
export default function AIVoiceSynth({
  text,
  platform,
  shouldPlay,
  whisperName,
  ...
  useEffect(() => {
    const speakText = async () => {
      ...
      let voiceUri: string = "";
      if (platform === "whisper") {
        voiceUri = (await whisperGetVoiceURI(
          text,
          whisperName as WhisperVoices
        )) as string;
      } else if (platform === "polly") {
        voiceUri = (await pollyGetVoiceURIFaster(
          text,
          pollyName as PollyVoices
        )) as string;
  ...
  return (
    <audio src={audioSource} id="audioPlayback" controls autoPlay hidden />
  );
}
