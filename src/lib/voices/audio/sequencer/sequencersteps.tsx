...
import { getDrumColor } from "../audioutils";
import React, { useEffect, useState } from "react";
import * as Tone from "tone";
import { Step, Song, SequenceRow } from "./sequencertypes";
...
export default function SequencerSteps({
          ...
          <div
            className={cn(
              `absolute flex items-center justify-center w-5 ${
                isActive ? "h-[40%]" : "h-[100%]"
              } md:w-12 md:p-2 mx-1 text-white border-[1px]? z-[50]? border-lime-300 rounded-none md:hover:bg-lime-800? cursor-pointer `
              // isActive && selectedColor,
              // rollActive && "border-4"
            )}
            ...
            <div
              className={cn(
                `absolute bottom-0 w-6 ${setTailWindVolume(
                  currentVolume
                )} md:w-10 md:p-2 mx-1 -z-[50] rounded-b-xl bg-white/10 border-t-2 border-lime-400/40 `
              )}
              onClick={updateThisPad}
            ></div>
          )}
  ...
  return (
    ...
    >
      <div 
      //className="flex rounded-xl flex-col p-3 bg-gradient-to-r from-violet-800 via-indigo-700 border-[1px] border-indigo-600 to-purple-500/10 w-full "
      >
        {sequences[0]! &&
              ...
              >
                ...
                <div>
                  <div className="relative">
                    <div
                      key={`row-${index}`}
                      className={`absolute flex gap-2 w-full flex-wrap? md:flex-nowrap items-center justify-center z-40 opacity-80 mx-auto md:flex-row m-4 `}
                    >
                        ...
                        return (
                          <>
                            ...
                            <KeyPad
                              key={`step-${stepNum}`}
                              {...keypad}
                              onPadClicked={() => updatePad(index, stepNum, "selected")}
