...
/**
 * Runs a multi-agent workflow where agents are processed in a RANDOM order
 * each round.
 *
...
export async function ORCHESTRATION_runRandomWorkflow(
    ...
    try {
        logger.log("Starting RANDOM_WORKFLOW", { config: orchState.config });
        ...
        for (orchState.currentRound = 0; orchState.currentRound < effectiveMaxRounds; orchState.currentRound++) {
            ...
            const shuffledAgents = shuffleArray([...agents]); // Shuffle a copy
            logger.log(`Round ${orchState.currentRound + 1} Agent Order: ${shuffledAgents.map(a => a.name).join(', ')}`);

            // Cycle through the SHUFFLED agents
            for (let i = 0; i < shuffledAgents.length; i++) {
                orchState.currentCycleStep = i; // Step relative to shuffled order
                ...
                const currentAgent = shuffledAgents[i]; // Get agent from shuffled list
                ORCHESTRATION_setActiveAgent(orchState, currentAgent);
                logger.log(`Round ${orchState.currentRound + 1}, Step ${i + 1}: Activating Agent ${currentAgent.name}`);

                // Determine message for agent (same logic as sequential/reverse)
                let messageForAgent: string;
                const isFirstTurnOverall = orchState.currentRound === 0 && i === 0;
        ...
        logger.error("Error during random workflow execution:", { error });
        orchState.status = "error";
