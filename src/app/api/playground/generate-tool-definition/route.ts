...
export async function POST(req: NextRequest) {
    try {
        ...
        logger.info("API: Received request to generate tool definition", { userId: body?.userId, toolName: body?.name });

        const validationResult = toolRequestSchema.safeParse(body);
        if (!validationResult.success) {
            logger.warn("API: Invalid request format for tool definition generation", { errors: validationResult.error.flatten() });
        ...
        const generatedDefinition = await CORE_generateCustomToolDefinition(toolRequestForCore, modelArgsForCore);
